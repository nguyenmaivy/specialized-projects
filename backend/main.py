...
# Ensure database objects exist when the app starts
@app.on_event("startup")
def _on_startup_initialize_db():
    try:
        if is_db_enabled():
            init_db()
...
def _compute_analysis_bundle(filters: FiltersPayload):
    ...
    #  What-if analysis: giả định tăng giảm discount sẽ ảnh hưởng thế nào đến sales và profit, đặc biệt với segment "At Risk"
    try:
    ...
    #  Kịch bản đơn giản hơn nếu dữ liệu không đủ hoặc có lỗi trong tính toán nhạy cảm với discount
    # try:
    #     margin = (kpis["total_profit"] / kpis["total_sales"]) if kpis["total_sales"] > 0 else 0
    #     discount_sensitivity = float(np.nan_to_num(filtered[["Discount", "Sales"]].corr().iloc[0, 1], nan=0.0))

    #     delta_discount_pct = 5.0   # Tăng từ 1.0 lên 5.0 để dễ thấy hơn
    #     uplift_sales = kpis["total_sales"] * (discount_sensitivity * (delta_discount_pct / 100))
    #     uplift_profit = uplift_sales * margin
    ...
    #             "uplift_profit": float(uplift_profit),
    #         },
    #         "at_risk_scenario": { ... }   # giữ nguyên
    #     }
    # except Exception as e:
    #     logger.warning(f"What-if calculation lỗi: {e}")
    #     what_if = {
    #         "discount_scenario": {
    #             "delta_discount_pct": 5.0,
    #             "uplift_sales": 0,
    #             "uplift_profit": 0,
    #         }
    #     }
...
def _default_widgets(evidence: dict, filters: Optional[Dict] = None):
    """Tạo insight tôn trọng bộ lọc (time range, category, region) - An toàn khi không có filter"""
    ...
    # Xử lý filters an toàn
    if filters is None:
        filters = {}

    category_filter = filters.get("category") if isinstance(filters.get("category"), list) else None
    region_filter = filters.get("region") if isinstance(filters.get("region"), list) else None
    start_date = filters.get("start_date")
    end_date = filters.get("end_date")

    # Tạo chuỗi hiển thị filter
    filter_text = ""
    filter_parts = []
    if category_filter:
        filter_parts.append(f"Category: {', '.join(category_filter)}")
    if region_filter:
        filter_parts.append(f"Region: {', '.join(region_filter)}")
    if start_date and end_date:
        filter_parts.append(f"Thời gian: {start_date} → {end_date}")
    elif start_date:
        filter_parts.append(f"Từ ngày: {start_date}")
    elif end_date:
        filter_parts.append(f"Đến ngày: {end_date}")

    if filter_parts:
        filter_text = " (" + " | ".join(filter_parts) + ")"

    # Xử lý trend theo tháng
    try:
        ...
        if not df_trend.empty and "Order Date" in df_trend.columns:
            ...
            if len(df_month) >= 1:
                ...
                if len(df_month) >= 2:
                    ...
                    valid_changes = df_month["pct_change"].dropna()
                    if not valid_changes.empty:
                        mom_max_row = df_month.loc[df_month["pct_change"].idxmax()]
                        mom_min_row = df_month.loc[df_month["pct_change"].idxmin()]
                        mom_max = (mom_max_row["Month"].strftime("%b %Y"), float(mom_max_row["pct_change"]))
                        mom_min = (mom_min_row["Month"].strftime("%b %Y"), float(mom_min_row["pct_change"]))

<<<<<<< HEAD
                trend_text = "tăng" if growth_pct > 0 else "giảm" if growth_pct < 0 else "ổn định"
    except Exception:
        pass  # fallback đã có ở trên
...
=======
            ...
            period_start = period_end = "Toàn bộ dữ liệu"
        ...
        growth_pct = 0
        trend_text = "không rõ"
>>>>>>> 9522aab2e190246f268106cb47cb6192a6f1420f
...
<<<<<<< HEAD
    lines = []
    lines.append(f"**Tổng doanh thu{filter_text} là ${total_sales:,.2f}**")
=======
    # Xây dựng nội dung
    lines = []
    # lines.append(f"**Tổng doanh thu từ {period_start} đến {period_end}{filter_text} là ${total_sales:,.2f}**")
    lines.append(f"**Tổng doanh thu {filter_text} là ${total_sales:,.2f}**")
>>>>>>> 9522aab2e190246f268106cb47cb6192a6f1420f
    lines.append("")
...
<<<<<<< HEAD
    # ==================== GỢI Ý CỤ THỂ HƠN ====================
    suggestion = "Xem xét **A/B test thay đổi discount**"
    if category_filter:
        top_cat = category_filter[0]  # lấy category đầu tiên người dùng đang lọc
        suggestion += f" tại **{top_cat}**"
    else:
        # Nếu không lọc category, lấy category có doanh thu cao nhất từ dữ liệu
        try:
            cat_sales = evidence.get("category_sales", [])
            if cat_sales:
                top_cat = max(cat_sales, key=lambda x: x.get("Sales", 0))["Category"]
                suggestion += f" tại **{top_cat}**"
        except:
            pass

    # Gợi ý % discount dựa trên discount_sensitivity (nếu có)
    try:
        sensitivity = float(what_if.get("discount_scenario", {}).get("discount_sensitivity", 0))
        if sensitivity > 0.3:
            suggestion += f" với mức tăng **+5-8%** discount"
        elif sensitivity > 0.1:
            suggestion += f" với mức tăng **+3-5%** discount"
    except:
        suggestion += " (khuyến nghị +3-5%)"

    # lines.append(f"• Gợi ý: {suggestion} để tối ưu lợi nhuận.")
    ...
    widgets = [
        {
            ...
            "evidence_json": {
                ...
                "filters_applied": bool(filter_parts)
            }
        }
    ]
# ==================== WHAT-IF WIDGET ====================
    what_if_widget_added = False
    try:
        if what_if and isinstance(what_if, dict):
            discount_scenario = what_if.get("discount_scenario")
            if discount_scenario and isinstance(discount_scenario, dict):
                uplift_sales = float(discount_scenario.get('uplift_sales', 0) or 0)
                uplift_profit = float(discount_scenario.get('uplift_profit', 0) or 0)

                if uplift_sales > 0 or uplift_profit > 0:   # Chỉ thêm nếu có giá trị ý nghĩa
                    widgets.append({
                        "widget_type": "what_if",
                        "severity": "medium",
                        "title": "What-if: Điều chỉnh discount",
                        "content_markdown": (
                            f"• Ước tính tăng doanh thu: **${uplift_sales:,.2f}**\n"
                            f"• Ước tính tăng lợi nhuận: **${uplift_profit:,.2f}**"
                        ),
                        "evidence_json": {"what_if": discount_scenario},
                    })
                    what_if_widget_added = True
    except Exception as e:
        logger.warning(f"Failed to create what_if widget: {e}")

    # Nếu không có what_if từ calculation, tạo một widget fallback đơn giản
    # if not what_if_widget_added:
    #     try:
    #         # Tính margin đơn giản để tạo gợi ý
    ...
    #             "title": "What-if: Điều chỉnh discount",
    #             "content_markdown": (
    #                 f"• Giả sử tăng discount 5%, ước tính lợi nhuận có thể thay đổi khoảng **{margin:.1f}%** của doanh thu.\n"
    #                 f"• Khuyến nghị thử nghiệm trên nhóm sản phẩm có biên lợi nhuận cao."
    #             ),
    #             "evidence_json": {"note": "fallback_when_no_sensitivity"}
    #         })
    #     except:
    #         pass

    # Fallback nếu không có dữ liệu what_if
    if not what_if_widget_added:
        widgets.append({
            "widget_type": "what_if",
            "severity": "medium",
            "title": "What-if: Điều chỉnh discount",
            "content_markdown": "• Chưa có đủ dữ liệu để tính toán kịch bản điều chỉnh discount.\n• Khuyến nghị thử nghiệm tăng/giảm discount 3-5% trên nhóm sản phẩm có biên lợi nhuận cao.",
            "evidence_json": {"note": "fallback"}
        })
...
def generate_ai_insights(request: Request, payload: dict, current_user: dict = Depends(get_current_user)):
    ...
    try:
        if payload.get("analysis_run_id"):
            ...
            # Pass through the stored filters so widgets honor the analysis-run filters
            filters_for_widgets = analysis_run.get("filters_json", {}) or {}
            widgets = _default_widgets(evidence, filters=filters_for_widgets)
            ...
            if client is not None:
                system_prompt = (
                    ...
                    "4. Dùng tiếng Việt, xuống dòng ở mỗi ý và in đậm các phần quan trọng.\n"
        ...
        if payload.get("chartId"):
            ...
            # Merge explicit timeRange into filters so chart-level requests that pass timeRange are respected
            time_range = payload.get("timeRange") or {}
            if isinstance(time_range, dict):
                if time_range.get("from") and "start_date" not in filters:
                    filters["start_date"] = time_range.get("from")
                if time_range.get("to") and "end_date" not in filters:
                    filters["end_date"] = time_range.get("to")
            ...
            # Ensure chart-level widgets respect filters passed from the frontend
            widgets = _default_widgets(evidence, filters=filters)
